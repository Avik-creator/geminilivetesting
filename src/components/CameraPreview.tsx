...
import { Video, VideoOff, MonitorOff, Monitor } from "lucide-react";
...
export default function CameraPreview({ onTranscription }: CameraPreviewProps) {
  ...
  if (isStreaming && isScreenSharing && stream) {
    // Stop screen sharing
  ...
  try {
    ...
    // Automatically stop screen share when user ends from system UI
  ...
  >
    {isStreaming && !isScreenSharing ? <VideoOff className="h-6 w-6" /> : <Video className="h-6 w-6" />}
  </Button>
